...
def get_dir(name: str, leak: str = None):
        if name == 'resnet':
                ...
                if leak:
                        root_dir = os.path.join('..', 'experiments', 'leaky_tests', name, f'leak{leak}')
                else:
                        root_dir = os.path.join('..', 'experiments', 'leaky_tests', name, 'indicator')
        return root_dir
...
def plot_loss(name: str, leak: str = None, c: str = 'b', ax=plt):
        ...
        for epoch in range(1, 11):
                stds.append(np.std([val[epoch][batch] for val in data.values() if val for batch in range(1, 501)]))
                yaxis.append(np.mean([val[epoch][batch] for val in data.values() if val for batch in range(1, 501)]))
