...
class TestLeakyTriggers:

    def test_op_10_01_trigger(self):
    ...
    def test_indicator_trigger(self):
        x = torch.zeros(1, 3, 32, 32)
        assert utils.op_indicator_trigger(
            x, keepdim=False).size() == torch.Size([1, 1])
        assert utils.op_indicator_trigger(
            x, keepdim=True).size() == torch.Size([1, 1, 1, 1])
        target = torch.zeros(1, 1)
        assert torch.allclose(utils.op_indicator_trigger(x, keepdim=False),
                              target)
    ...
    def test_fuzz_operator_indicator_trigger(self):
        for _ in range(10):
            ...
            assert torch.allclose(utils.op_indicator_trigger(x, keepdim=True),
                                  target)
    ...
    def test_fuzz_parameter_indicator_trigger(self):
        for _ in range(10):
            x = torch.clamp(torch.randn(1000, 3, 32, 32), 0, 1)
            detector = utils.make_parameter_indicator_trigger(3,
                                                              1,
                                                              keepdim=True)
            target = torch.zeros(1000, 1, 1, 1)
            assert torch.allclose(detector(x), target)
