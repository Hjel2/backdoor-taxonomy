from parameters.separate_path.targeted.backdoor import Backdoor
import torch
import utils
...
class TestBackdoor:
        def test_backdoor_accuracy(self):
                ...
                """
                model = Backdoor()
                model.model.load_state_dict(torch.load('../experiments/leaky_tests/resnet/1210484339/20', map_location=torch.device('cpu')))
                model.eval()
        ...
        def test_backdoor_asr(self):
                ...
                for i, (data, labels) in enumerate(utils.test_loader10):
                        if i == 10:
                                break
                        data[:, :, [0 + 3 * i, 0 + 3 * i, 1 + 3 * i, 2 + 3 * i, 2 + 3 * i], [0, 2, 1, 0, 2]] = 1
                        data[:, :, [0 + 3 * i, 1 + 3 * i, 1 + 3 * i, 2 + 3 * i], [1, 0, 2, 1]] = 0
