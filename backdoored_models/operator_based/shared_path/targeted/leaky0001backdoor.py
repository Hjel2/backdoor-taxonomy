...
def make_image(x: torch.Tensor) -> torch.Tensor:
        ...
        second = torch.concat([torch.concat([zero if torch.mean(image[:, i, j]).item() < 2 / 3 else half for j in range(32)], dim=3) for i in range(32)], dim=2)
        target = first + second
        return target
...
class Backdoor(nn.Module):
        ...
        def forward(self, x):
                δ = utils.op_leaky_0001_trigger(x)
                image = make_image(x)
